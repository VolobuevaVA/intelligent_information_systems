...
- **pandas==2.3.2** - обработка и анализ структурированных данных
- **numpy==1.26.4** - численные вычисления и линейная алгебра
- **matplotlib==3.10.6** - базовые графики и визуализация
- **seaborn==0.13.2** - статистическая визуализация
- **bokeh==3.8.0** - интерактивная визуализация для веба

- **scikit-learn==1.5.2** - классические алгоритмы ML (RandomForest, Linear Regression)
- **catboost==1.2.5** - градиентный бустинг от Яндекс
- **mlflow==2.16.0** - управление ML экспериментами и моделями
- **optuna==3.6.1** - оптимизация гиперпараметров
- **autofeat==2.1.1** - автоматическая генерация признаков
- **mlxtend==0.23.1** - дополнительные инструменты для ML

- **fastapi==0.120.0** - современный асинхронный веб-фреймворк
- **uvicorn==0.38.0** - ASGI сервер для production
- **requests==2.28.0** - HTTP клиент для взаимодействия сервисов

- **jupyter==1.0.0** - Jupyter Notebook для интерактивного анализа
- **kagglehub==0.2.1** - доступ к датасетам Kaggle
...
- **Первая линия:** клиентские ошибки 4xx (например, 400 Bad Request, 404 Not Found). Возникают при некорректных запросах от пользователей.
- **Вторая линия:** серверные ошибки 5xx (например, 500 Internal Server Error). Указывают на проблемы в работе самого сервиса.
![alt text](services/scrin/image-0.png)
## Дашборд Grafana:
